...
import ru.qmbo.mc1.service.SessionHandlerService;
...
public class WebSocketClientConfig {
    ...
    public WebSocketStompClient webSocketStompClient(
            WebSocketClient webSocketClient, SessionHandlerService sessionHandlerNoResponse,
            @Value("${websocket.server.url-no-response}") String urlEndpointNoResponse) {
        WebSocketStompClient stompClient = new WebSocketStompClient(webSocketClient);
        stompClient.setMessageConverter(new MappingJackson2MessageConverter());
        stompClient.setTaskScheduler(new ConcurrentTaskScheduler());
        stompClient.connect(urlEndpointNoResponse, sessionHandlerNoResponse);
        return stompClient;
    }
